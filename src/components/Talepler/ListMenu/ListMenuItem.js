...
const ListMenuItem = ({ items, ...props }) => {
    ...
    const [tab, setTab] = useState({ route: path[1] })
...
console.log(tab)
    ...
    return (
        <Fragment>
            <Box textAlign={"center"} fontSize="33px" p="17px" bg={"blue"}>{tab && tab.title + " >>"}{tab2 && tab2.title + " >>"}{tab3 && tab3.title + " >>"}{tab4 && tab4.title + " >>"}{tab5 && tab5.title + " >>"}{tab6 && tab6.title + ">>"}</Box>
                    ...
                    {items.map((menu, index) => {
                        return (
                            <li className="menuItem" key={index} style={{ marginLeft: "12px" }}>
                                {console.log(menu)}
                                <CLink as={RLink} color={tab?.route == menu.route ? colors.yellow : "white"} onClick={() => tab == menu.title ? close1() : setTab(menu)} to={`/${menu.route}/`}>{menu.title}</CLink >
                                {/* {tab == menu.title &&
                                    <ul>
