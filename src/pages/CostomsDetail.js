...
const CostomsDetail = () => {
  ...
  return (
    <Box w="100%">
      <BreadCrumb>Maliyetleri</BreadCrumb>
        ...
        <Box w="100%" textAlign={"center"} mb="40px">
          {Tabs2?.map((tab) => (
            <Button
              ...
              borderRadius={"10px"}
              bg={
                tab.title == activeTab2.title ? colors.lightdarkblue : "white"
              }
              onClick={() => setActiveTab2(tab)}
              fontSize="18px"
              color={
                tab.title == activeTab2.title ? "white" : colors.lightdarkblue
              }
            >
              {tab.title}
            </Button>
